 ...
 * the next row, and because it returns false when there are no more rows in the Cursor object, it
 * can be used in a while loop to iterate through the contents.</p>
 *
 * <p>A cursor instance <b>IS NOT</b> thread-safe and should be used only from a single thread.
 * Multiple cursor instances may be used in their own threads.</p>
 */
public interface Cursor extends CursorEntry {
   ...
   * makes the first row the current row; the second call makes the second row the current row, and
   * so on.</p>
   *
